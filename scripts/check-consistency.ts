...
async function checkConsistency() {
  ...
  try {
  // 1. 从 OKX 获取实际持仓
  console.log("🔍 步骤1: 获取 OKX 实际持仓...");
    const okxPositions = await okxClient.getPositions();
    const activeOkxPositions = okxPositions.filter(
      (p: any) => Number.parseInt(p.size || "0") !== 0
    );

    console.log(`   ✅ OKX 当前持仓数: ${activeOkxPositions.length}\n`);

    if (activeOkxPositions.length > 0) {
      for (const pos of activeOkxPositions) {
  ...
  console.log(`      OKX size值: ${size} (${size > 0 ? "正数=做多" : "负数=做空"})\n`);
      }
    }
    ...
    // 3. 对比一致性
    console.log("🔍 步骤3: 对比 OKX 与数据库持仓一致性...\n");

    const okxSymbols = new Set(
      activeOkxPositions.map((p: any) => p.contract.replace("_USDT", ""))
    );
  ...
  // 检查 OKX 有但数据库没有的
  const missingInDb = Array.from(okxSymbols).filter(s => !dbSymbols.has(s));
    if (missingInDb.length > 0) {
  console.log(`   ⚠️ OKX 有但数据库缺失的持仓: ${missingInDb.join(", ")}`);
    }

  // 检查数据库有但 OKX 没有的
    const missingInOkx = Array.from(dbSymbols).filter(s => !okxSymbols.has(s));
    if (missingInOkx.length > 0) {
      console.log(`   ⚠️ 数据库有但 OKX 已平仓的持仓: ${missingInOkx.join(", ")}`);
    }

    // 检查两边都有的，对比详细信息
  const commonSymbols = Array.from(okxSymbols).filter(s => dbSymbols.has(s));
    for (const symbol of commonSymbols) {
      const okxPos = activeOkxPositions.find(
  ...
  const okxSize = Number.parseInt(okxPos.size || "0");
  const okxSide = okxSize > 0 ? "long" : "short";
  const okxQuantity = Math.abs(okxSize);

      const inconsistencies: string[] = [];

      if (okxSide !== dbPos.side) {
        inconsistencies.push(
          `方向不一致: OKX=${okxSide}, DB=${dbPos.side}`
        );
      }

      if (okxQuantity !== dbPos.quantity) {
        inconsistencies.push(
          `数量不一致: OKX=${okxQuantity}, DB=${dbPos.quantity}`
        );
      }
  ...
  console.log("   - OKX 的 size 字段: 正数=做多，负数=做空\n");
