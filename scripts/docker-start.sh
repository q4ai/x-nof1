...
# 检查必需的环境变量
echo "🔍 检查环境变量配置..."
if ! grep -q "OKX_API_KEY=your_okx_api_key_here" .env && \
   ! grep -q "OKX_API_SECRET=your_okx_api_secret_here" .env && \
   ! grep -q "OPENAI_API_KEY=your_api_key_here" .env; then
    echo "✅ 环境变量已配置"
else
...
if [[ $REPLY == "2" ]]; then
    ...
    if grep -q "OKX_USE_PAPER=true" .env; then
        echo "⚠️  警告: 生产环境检测到 OKX_USE_PAPER=true"
        read -p "是否继续使用模拟盘? (y/N) " -n 1 -r
        echo
        if [[ ! $REPLY =~ ^[Yy]$ ]]; then
            echo "请修改 .env 文件: OKX_USE_PAPER=false"
            exit 0
        fi
    fi
fi
