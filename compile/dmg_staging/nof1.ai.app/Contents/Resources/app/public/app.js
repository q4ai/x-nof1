    ...
    try {
        ...
        container.innerHTML = data.positions.map(pos => {
            ...
            const contractsLabel = Number.isFinite(contractsValue) && contractsValue > 0
                ? `${formatQuantity(contractsValue)} 张`
                : '';
            const openValue = Number(pos.openValue);
            ...
            const profitTarget = Number(pos.profitTarget);
            ...
            <div class="position-item ${pos.side}">
                ...
                <div class="position-grid">
                    <div class="position-field">
                        ...
                        <div class="value"${contractsLabel ? ` title="${contractsLabel}"` : ''}>${quantityLabel} ${symbol}</div>
                    </div>
                    <div class="position-field">
                        <div class="label">开仓价</div>
                        <div class="value">${Number.isFinite(entryPrice) ? entryPrice.toFixed(4) : '--'}</div>
                    </div>
                    <div class="position-field">
                        <div class="label">开仓价值</div>
                        <div class="value">${Number.isFinite(openValue) ? openValue.toFixed(2) : '--'} USDT</div>
                    </div>
                    <div class="position-field">
                        <div class="label">当前价</div>
                        <div class="value">${Number.isFinite(currentPrice) ? currentPrice.toFixed(4) : '--'}</div>
                    </div>
                    <div class="position-field">
                    ...
                    <div class="position-field">
                        ...
                        <div class="value">${Number.isFinite(liquidationPrice) ? liquidationPrice.toFixed(4) : '--'}</div>
                    </div>
                    ${pos.stopLoss ? `
                    <div class="position-field">
                        <div class="label">止损</div>
                        <div class="value">${Number.isFinite(stopLoss) ? stopLoss.toFixed(4) : '--'}</div>
                    </div>
                    ` : ''}
                    ${pos.profitTarget ? `
                    <div class="position-field">
                        <div class="label">止盈</div>
                        <div class="value">${Number.isFinite(profitTarget) ? profitTarget.toFixed(4) : '--'}</div>
                    </div>
                    ` : ''}
                </div>
            </div>
                ...
                <div class="trade-item">
                    ...
                    <div class="trade-info">
                        ...
                        <div class="trade-field">
                            ...
                            <span class="value"${contractsLabel ? ` title="${contractsLabel}"` : ''}>${quantityLabel} ${symbol}</span>
                        </div>
                        <div class="trade-field">
                            <span class="label">价格</span>
                            <span class="value">${Number.isFinite(priceValue) ? priceValue.toFixed(4) : '--'}</span>
                        </div>
                        <div class="trade-field">
                        ...
                        <div class="trade-field">
                            ...
                            <span class="value">${Number.isFinite(feeValue) ? feeValue.toFixed(4) : '--'}</span>
                        </div>
                        ${pnlHtml}
                    </div>
                </div>
...
// 加载交易执行日志
async function loadTradeLogsData() {
    try {
        const response = await fetch('/api/trade-logs?limit=100');
        const data = await response.json();
        ...
        if (!data.logs || data.logs.length === 0) {
            container.innerHTML = '<p class="no-data">暂无交易日志</p>';
        ...
        container.innerHTML = data.logs.map((log, index) => {
            ...
            const actionLabel = mapTradeAction(log.action);
            const statusLabel = mapTradeStatus(log.status);
            const statusClass = `trade-log-status ${log.status || 'unknown'}`;
            const sideLabel = log.side ? (log.side === 'long' ? '多' : '空') : null;
            const leverageLabel = typeof log.leverage === 'number' && !Number.isNaN(log.leverage)
                ? `${log.leverage}x`
                : null;
            const sizeLabel = typeof log.size === 'number' && !Number.isNaN(log.size)
                ? log.size.toFixed(2)
                : null;
            const amountLabel = typeof log.amountUsdt === 'number' && !Number.isNaN(log.amountUsdt)
                ? `${log.amountUsdt.toFixed(2)} USDT`
                : null;

            const requestDetails = formatJsonForDisplay(log.rawRequest);
            const responseDetails = formatJsonForDisplay(log.rawResponse);
            const hasDetails = requestDetails || responseDetails;

            return `
                <div class="trade-log-item ${log.status}">
                    <div class="trade-log-header">
                        <div class="trade-log-header-left">
                            <span class="trade-log-time">${timeStr}</span>
                            <span class="trade-log-separator">·</span>
                            <span class="trade-log-action">${actionLabel}</span>
                            ${log.symbol ? `<span class="trade-log-symbol">${log.symbol}</span>` : ''}
                            ${sideLabel ? `<span class="trade-log-tag ${log.side}">${sideLabel}</span>` : ''}
                            ${leverageLabel ? `<span class="trade-log-tag">杠杆 ${leverageLabel}</span>` : ''}
                            ${sizeLabel ? `<span class="trade-log-tag">数量 ${sizeLabel}</span>` : ''}
                            ${amountLabel ? `<span class="trade-log-tag">名义金额 ${amountLabel}</span>` : ''}
                        </div>
                        <span class="${statusClass}">${statusLabel}</span>
                    </div>
                    <div class="trade-log-message">${escapeHtml(log.message || '-')}</div>
                    ${(log.orderId || hasDetails) ? `
                        <div class="trade-log-meta">
                            ${log.orderId ? `<div class="trade-log-meta-item"><span class="label">订单ID</span><span class="value">${escapeHtml(log.orderId)}</span></div>` : ''}
                        </div>
                    ` : ''}
                    ${hasDetails ? `
                        <details class="trade-log-details" id="trade-log-details-${index}">
                            <summary>查看原始响应</summary>
                            ${requestDetails ? `<div class="trade-log-detail-block"><div class="trade-log-detail-title">请求</div><pre>${requestDetails}</pre></div>` : ''}
                            ${responseDetails ? `<div class="trade-log-detail-block"><div class="trade-log-detail-title">响应</div><pre>${responseDetails}</pre></div>` : ''}
                        </details>
                    ` : ''}
                </div>
            `;
        ...
        window.tradeLogsData = data.logs;
    } catch (error) {
...
function mapTradeAction(action) {
    switch (action) {
        case 'open':
            return '开仓';
        case 'close':
...
function copyLog(index) {
    ...
    navigator.clipboard.writeText(logText).then(() => {
        ...
        if (btn) {
            const originalHTML = btn.innerHTML;
            btn.innerHTML = '<span style="color:#ef4444;">复制失败</span>';
