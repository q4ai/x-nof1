 ...
 * 从 OKX 获取合约乘数
 */
async function fetchContractMultipliers(): Promise<
	Map<string, { multiplier: number; contractValue: string }>
> {
	const okxClient = createOkxClient();
	const multipliers = new Map<
		string,
		{ multiplier: number; contractValue: string }
	>();
 ...
 * 保存合约乘数到数据库
 */
async function saveContractMultipliers(
	multipliers: Map<string, { multiplier: number; contractValue: string }>,
): Promise<void> {
	const now = new Date().toISOString();

	try {
		logger.info(
			`Saving ${multipliers.size} contract multipliers to database...`,
		);
 ...
 * 从数据库获取所有合约乘数
 */
export async function getContractMultipliersFromDb(): Promise<
	ContractMultiplier[]
> {
	try {
		const result = await dbClient.execute(
			"SELECT * FROM contract_multipliers ORDER BY symbol",
		);
 ...
 */
export function startContractMultiplierSync(
	intervalHours = 24,
): NodeJS.Timeout {
	logger.info(
		`Starting contract multiplier sync scheduler (every ${intervalHours} hours)`,
	);
	...
	logger.info(
		`Contract multiplier sync scheduler started, next sync in ${intervalHours} hours`,
	);
	return timer;
}
