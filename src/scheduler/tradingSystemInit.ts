...
import { getAccountRiskConfig, type AccountRiskConfig } from "../agents/tradingAgent";
...
async function syncRiskConfigToDatabase(config: AccountRiskConfig): Promise<void> {
  try {
    ...
    await dbClient.execute({
      ...
      args: ["account_stop_loss_usdt", String(config.stopLossUsdt ?? 0), timestamp],
    });
    await dbClient.execute({
      sql: `INSERT OR REPLACE INTO system_config (key, value, updated_at) VALUES (?, ?, ?)`,
      args: ["account_take_profit_usdt", String(config.takeProfitUsdt ?? 0), timestamp],
    });
    logger.info(
...
async function loadRiskConfigFromDatabase(): Promise<AccountRiskConfig | null> {
  try {
    ...
    const stopLossValue = Number.parseFloat(String(stopLossResult.rows[0]?.value ?? "0"));
    const takeProfitValue = Number.parseFloat(String(takeProfitResult.rows[0]?.value ?? "0"));

    return {
      stopLossUsdt: Number.isFinite(stopLossValue) ? stopLossValue : undefined,
      takeProfitUsdt: Number.isFinite(takeProfitValue) ? takeProfitValue : undefined,
      syncOnStartup: false,
    };
  } catch (error) {
    logger.warn("从数据库读取账户风控配置失败，使用当前内存配置:", error as Error);
    return null;
  }
}
...
export async function initTradingSystem(): Promise<void> {
  ...
  const exchangeProvider = (process.env.EXCHANGE_PROVIDER || "okx").toLowerCase();
