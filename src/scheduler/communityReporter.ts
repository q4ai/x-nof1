...
const REPORT_ENDPOINT =
	process.env.COMMUNITY_REPORT_URL || "http://report.q4.net/api/v1/competition";
const REPORT_TIMEOUT_MS = Number.parseInt(
	process.env.COMMUNITY_REPORT_TIMEOUT_MS || "15000",
	10,
);
const HISTORY_LIMIT = Number.parseInt(
	process.env.COMMUNITY_REPORT_HISTORY_LIMIT || "500",
	10,
);
const TRADES_LIMIT = Number.parseInt(
	process.env.COMMUNITY_REPORT_TRADES_LIMIT || "100",
	10,
);
...
async function runCommunityReport() {
	...
	try {
		...
		if (
			!config ||
			String(config.COMMUNITY_REPORT_ENABLED).toLowerCase() !== "true"
		) {
		...
		const sharePrompts =
			String(config.COMMUNITY_SHARE_PROMPTS).toLowerCase() === "true";
...
async function fetchAccountSnapshot(
	...
	try {
		...
		logger.warn(
			"Failed to fetch OKX account for community report, falling back to history snapshot",
			error as any,
		);
...
async function fetchLatestAccountHistorySnapshot(
	statistics: TradingStatistics,
): Promise<AccountSnapshot | null> {
...
async function fetchTradingStatistics(): Promise<TradingStatistics> {
	...
	const initialBalance =
		history.length > 0 ? toNumber(history[0].total_value, 100) : 100;
	const currentBalance =
		history.length > 0
			? toNumber(history[history.length - 1].total_value, initialBalance)
			: initialBalance;
	...
	const totalProfit = winTrades.reduce(
		(sum, row) => sum + toNumber(row.pnl),
		0,
	);
	const totalLossAbs = Math.abs(
		lossTrades.reduce((sum, row) => sum + toNumber(row.pnl), 0),
	);
	...
	const maxWin =
		winTrades.length > 0
			? Math.max(...winTrades.map((row) => toNumber(row.pnl)))
			: 0;
	const maxLoss =
		lossTrades.length > 0
			? Math.abs(Math.min(...lossTrades.map((row) => toNumber(row.pnl))))
			: 0;
	...
	if (returns.length > 0) {
		...
		if (downReturns.length > 0) {
			const downVariance =
				downReturns.reduce((sum, value) => sum + value ** 2, 0) /
				downReturns.length;
	...
	const payload: ReportPayload = {
		...
		config: {
			...
			intervalMinutes: Number.parseInt(
				config.TRADING_INTERVAL_MINUTES || "20",
				10,
			),
			symbols,
...
async function sendReport(payload: ReportPayload) {
	...
	try {
		...
		if (!response.ok) {
			...
			throw new Error(
				`Report request failed: ${response.status} ${response.statusText} ${text}`,
			);
		}
	} finally {
