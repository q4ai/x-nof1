...
async function executeInstance(
	...
	if (!acquireInstanceLock(instanceId)) {
		logger.warn(
			`实例 ${instanceName} (ID: ${instanceId}) 正在执行中，跳过本次调度`,
		);
		return;
	}
	...
	try {
		...
		logger.info(
			`  账户: ${instance.account_name} (${instance.account_provider})`,
		);
		...
		// 广播实例执行状态
		websocketService.pushInstanceStatus(
			instanceId,
			"executing",
			`Instance ${instanceName} is executing`,
		);

		// 动态导入执行器（避免循环依赖）
		const { executeInstanceTradingDecision } = await import(
			"./instanceExecutor"
		);
		...
		websocketService.pushInstanceStatus(
			instanceId,
			"idle",
			`Instance ${instanceName} completed`,
		);
	} catch (error) {
...
async function checkAndExecuteInstances(): Promise<void> {
	try {
		...
		logger.info(
			`调度 ${instancesToExecute.length}/${runningInstances.length} 个实例执行`,
		);
...
export async function getMultiInstanceStatus(): Promise<{
	...
	const runningInstances = await getRunningInstances();
	const executingCount = Array.from(instanceExecutionLocks.values()).filter(
		(v) => v,
	).length;
