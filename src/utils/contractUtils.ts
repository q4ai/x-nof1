 ...
 * @returns 合约乘数
 */
async function getMultiplierFromDatabase(symbol: string): Promise<number | null> {
  try {
...
export async function getQuantoMultiplier(
  ...
  if (typeof dbMultiplier === "number" && Number.isFinite(dbMultiplier) && dbMultiplier > 0) {
  ...
  try {
    ...
    const multiplier = Number.parseFloat(String(contractInfo.quantoMultiplier ?? ""));
