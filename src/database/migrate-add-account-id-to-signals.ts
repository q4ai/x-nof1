	...
	try {
		...
		try {
			await dbClient.execute(
				"ALTER TABLE trading_signals ADD COLUMN account_id INTEGER",
			);
		...
		try {
			...
			if (activeAccountResult.rows.length > 0) {
				...
				if (activeAccountId > 0) {
					logger.info(
						`正在将 trading_signals 现有数据的 account_id 更新为当前活跃账户 ID: ${activeAccountId}`,
					);
					await dbClient.execute({
						sql: "UPDATE trading_signals SET account_id = ? WHERE account_id IS NULL",
