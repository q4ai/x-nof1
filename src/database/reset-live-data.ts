...
const MEMORY_DB_URL =
	process.env.TRADING_MEMORY_DB_URL || "file:./data/database/trading-memory.db";
...
export async function resetLiveDataToDefaults(
	targetAccount?: AccountConfig | null,
): Promise<ResetLiveDataResult> {
	...
	const configuredInitialRaw =
		defaults.INITIAL_BALANCE || process.env.INITIAL_BALANCE || "1000";
	const configuredInitialParsed = Number.parseFloat(configuredInitialRaw);
	const configuredInitialBalance = Number.isFinite(configuredInitialParsed)
		? configuredInitialParsed
		: 1000;
	...
	try {
		...
		const safeTotal = Number.isFinite(accountTotal)
			? accountTotal
	...
	try {
		...
		try {
			...
			if (clearedMemoryTables.length > 0) {
				logger.info(
					`[reset] 已清空 AI 记忆库表: ${clearedMemoryTables.join(", ")}`,
				);
			}
		} catch (memoryError) {
