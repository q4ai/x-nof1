...
const DEFAULT_CONFIG = {
  ...
  // 回撤风控
  ACCOUNT_DRAWDOWN_WARNING_PERCENT: process.env.ACCOUNT_DRAWDOWN_WARNING_PERCENT || "20",
  ACCOUNT_DRAWDOWN_NO_NEW_POSITION_PERCENT: process.env.ACCOUNT_DRAWDOWN_NO_NEW_POSITION_PERCENT || "30",
  ACCOUNT_DRAWDOWN_FORCE_CLOSE_PERCENT: process.env.ACCOUNT_DRAWDOWN_FORCE_CLOSE_PERCENT || "50",
...
export async function getAllConfig(): Promise<Record<string, string>> {
  try {
    const result = await dbClient.execute("SELECT key, value FROM system_config");
...
export async function setConfigValue(key: string, value: string): Promise<void> {
  try {
    await dbClient.execute({
      ...
      args: [key, value, new Date().toISOString(), value, new Date().toISOString()],
    });
