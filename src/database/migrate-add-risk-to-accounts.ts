...
async function main() {
  ...
  try {
    ...
    const tableInfo = await client.execute("PRAGMA table_info(account_configs)");
    const columns = tableInfo.rows.map((row: any) => row.name);
    
    if (!columns.includes("stop_loss_usdt")) {
      await client.execute("ALTER TABLE account_configs ADD COLUMN stop_loss_usdt REAL");
    ...
    if (!columns.includes("take_profit_usdt")) {
      await client.execute("ALTER TABLE account_configs ADD COLUMN take_profit_usdt REAL");
