...
export async function ensureDualPositionSupport(client: Client): Promise<void> {
  try {
    // 检查是否已经是新结构
    const indexInfo = await client.execute("SELECT sql FROM sqlite_master WHERE type='table' AND name='positions'");
    if (indexInfo.rows.length > 0) {
      const createSql = (indexInfo.rows[0] as any).sql || "";
      // 如果已经包含 UNIQUE(symbol, side)，说明已迁移
      if (createSql.includes("UNIQUE(symbol, side)") || createSql.includes("UNIQUE (symbol, side)")) {
