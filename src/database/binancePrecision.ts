...
export async function upsertBinancePrecisions(
	records: BinancePrecisionRecord[],
): Promise<void> {
	if (!records.length) {
		return;
	}
...
export async function getBinancePrecision(
	contract: string,
): Promise<BinanceContractPrecision | null> {
...
export function computePrecisionFromStep(
...
export function normalizeBinanceQuantity(
	quantity: number,
	precision: number,
	stepSize: number,
): number {
...
export function formatBinanceQuantity(
	contract: string,
	quantity: number,
	precisionRecord?: BinanceContractPrecision,
): string {
	if (!precisionRecord) {
	...
	const normalized = normalizeBinanceQuantity(
		quantity,
		precision,
		Number.isFinite(step) && step > 0 ? step : Number.NaN,
	);
