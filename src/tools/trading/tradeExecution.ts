...
function providerUsesContractUnits(provider: ExchangeProvider): boolean {
	return CONTRACT_UNIT_PROVIDERS.has(provider);
}
...
async function resolveMinHoldingMinutes(): Promise<number> {
	const context = getCurrentInstanceContext();
	if (!context?.strategyName) {
		throw new Error("实例缺少策略名称，无法读取最小持仓时间参数");
	}

	const strategy = await StrategyFileManager.loadStrategy(context.strategyName);
	if (!strategy) {
		throw new Error(`无法加载策略 ${context.strategyName}，无法获取最小持仓时间`);
	}

	const configured = strategy.params?.minHoldingMinutes;
	const parsed = Number(configured);
	if (!Number.isFinite(parsed) || parsed <= 0) {
		throw new Error(
			`策略 ${context.strategyName} 未配置有效的最小持仓时间（当前值: ${configured}）`,
		);
	}
...
async function buildLotSizingInfo(
	...
	return {
		...
		lotSizePrecision,
		minSizeRaw:
			Number.isFinite(minSizeRaw) && minSizeRaw > 0 ? minSizeRaw : lotSize,
		maxSizeRaw:
...
async function getBinanceOrderRealizedPnl(
	...
	try {
		...
		const total = matched.reduce((sum: number, trade: any) => {
			const raw =
				trade?.realizedPnl ?? trade?.realisedPnl ?? trade?.realizedPNL;
		...
		logger.warn(
			`获取 Binance realizedPnl 失败(order ${orderId}): ${error instanceof Error ? error.message : String(error)}`,
		);
		return null;
	}
}
...
export async function executeOpenPosition({
	...
	const useContractUnits = providerUsesContractUnits(exchangeProvider);
	...
	logger.info(
		`🚀 [开仓请求] ${normalizedSymbol} ${side} | 金额: ${effectiveAmount} ${amountUnit} | 杠杆: ${leverage}x | 订单类型: ${orderType} | 是否面值: ${isNotional}`,
	);
	...
	try {
		...
		if (
			...
			return fail(
				`无效的杠杆倍数: ${leverage}（必须在1-${RISK_PARAMS.MAX_LEVERAGE}之间，最大值由环境变量MAX_LEVERAGE控制）`,
			);
		}
		...
		const quantoMultiplier =
			Number.isFinite(infoMultiplier) && infoMultiplier > 0
				? infoMultiplier
		...
		const quantityDivider = useContractUnits ? quantoMultiplier : 1;
		if (amountUnit === "coin") {
			// Input is Coin Quantity
			quantity = effectiveAmount / quantityDivider;
			notionalUsdt = effectiveAmount * executionPrice;
			marginUsdt = notionalUsdt / leverage;
			logger.info(
				`💡 数量计算 (币本位): 数量=${effectiveAmount} ${symbol} ÷ 乘数=${quantityDivider} = ${quantity.toFixed(4)} ${unitLabel}`,
			);
		} else {
			// Input is USDT
			if (isNotional) {
				...
				marginUsdt = notionalUsdt / leverage;
				quantity = notionalUsdt / (quantityDivider * executionPrice);
				logger.info(
					`💡 数量计算 (USDT面值): 面值=${effectiveAmount} USDT ÷ (乘数=${quantityDivider} × 价格=${executionPrice.toFixed(2)}) = ${quantity.toFixed(4)} ${unitLabel}`,
				);
			} else {
				// Margin (Old behavior)
				...
				quantity = notionalUsdt / (quantityDivider * executionPrice);
				logger.info(
					`💡 数量计算 (USDT保证金): 保证金=${effectiveAmount} USDT × 杠杆=${leverage}x ÷ (乘数=${quantityDivider} × 价格=${executionPrice.toFixed(2)}) = ${quantity.toFixed(4)} ${unitLabel}`,
				);
			}
		}
		...
		const activePositions = allPositions.filter(
			(p: any) => Math.abs(Number.parseFloat(p.size || "0")) > 0,
		);

		if (activePositions.length >= RISK_PARAMS.MAX_POSITIONS) {
			return fail(
				`已达到最大持仓数量限制（${RISK_PARAMS.MAX_POSITIONS}个），当前持仓 ${activePositions.length} 个，无法开新仓`,
			);
		}
		...
		if (existingPosition) {
			...
			if (existingSide !== side) {
				return fail(
					`${normalizedSymbol} 已有${existingSide === "long" ? "多" : "空"}单持仓，禁止同时持有双向持仓。请先平掉${existingSide === "long" ? "多" : "空"}单后再开${side === "long" ? "多" : "空"}单。`,
				);
			}

			// 如果方向相同，允许加仓（但需要注意总持仓限制）
			logger.info(
				`${normalizedSymbol} 已有${side === "long" ? "多" : "空"}单持仓，允许加仓`,
			);
		}
		...
		for (const pos of activePositions) {
			...
			// 获取合约乘数
			const posQuantoMultiplier = useContractUnits
				? await getQuantoMultiplier(pos.contract)
				: 1;
		...
		if (totalExposure > maxAllowedExposure) {
			return fail(
				`新开仓将导致总敞口 ${totalExposure.toFixed(2)} USDT 超过限制 ${maxAllowedExposure.toFixed(2)} USDT（账户净值的${RISK_PARAMS.MAX_LEVERAGE}倍），拒绝开仓`,
			);
		}
		...
		if (marginUsdt > maxSinglePosition) {
			logger.warn(
				`开仓保证金 ${marginUsdt.toFixed(2)} USDT 超过建议仓位 ${maxSinglePosition.toFixed(2)} USDT（账户净值的30%）`,
			);
		}
		...
		if (!Number.isFinite(quantity) || quantity <= 0) {
			return fail(
				`无法计算下单数量，请检查参数（amount=${effectiveAmount}, leverage=${leverage}, price=${currentPrice})`,
			);
		}
		...
		if (quantity < minSize) {
			const requiredMargin =
				(minSize * quantityDivider * currentPrice) / leverage;
			return fail(
				`计算的数量 ${quantity.toFixed(4)}${unitLabel} 低于最小下单单位 ${minSize.toFixed(4)}，至少需要 ${requiredMargin.toFixed(2)} USDT 保证金（当前${marginUsdt.toFixed(2)} USDT，杠杆${leverage}x）。`,
			);
		}
		...
		logger.info(
			`开仓 ${symbol} ${side === "long" ? "做多" : "做空"} ${Math.abs(size)}${unitLabel} (杠杆${leverage}x)`,
		);
		...
		try {
			...
			if (
				...
				(error.message.includes("-1111") ||
					error.message.includes("Precision is over the maximum")) &&
				Math.abs(size) >= 1 &&
		...
		if (order.id) {
			...
			while (retryCount < maxRetries) {
				try {
					const orderDetail = await client.getOrder(
						order.id.toString(),
						contract,
						order.clientOrderId,
					);
					...
					logger.info(
						`订单状态: ${finalOrderStatus}, 成交: ${actualFillSize}${unitLabel} @ ${actualFillPrice.toFixed(2)} USDT`,
					);
					...
					if (orderType === "market" && actualFillSize > 0) {
						...
						if (priceDeviation > 0.02) {
							// 滑点超过2%，拒绝此次交易（回滚）
							logger.error(
								`❌ 成交价偏离超过2%: ${currentPrice.toFixed(2)} → ${actualFillPrice.toFixed(2)} (偏离 ${(priceDeviation * 100).toFixed(2)}%)，拒绝交易`,
							);
							...
							return fail(
								`开仓失败：成交价偏离超过2% (${currentPrice.toFixed(2)} → ${actualFillPrice.toFixed(2)})，已拒绝交易`,
								{
					...
					if (retryCount >= maxRetries) {
						...
						// 如果无法获取订单详情，使用预估值继续
						logger.warn(
							`使用预估值继续: 数量=${Math.abs(size)}, 价格=${actualFillPrice}`,
						);
		...
		// 计算手续费
		const positionValue = finalQuantity * quantityDivider * actualFillPrice;
		...
		setTimeout(async () => {
			try {
				const { syncPositionsFromOkx } = await import(
					"../../database/sync-positions-only"
				);
...
export const openPositionTool = createTool({
	...
	parameters: z.object({
		...
		amountUsdt: z
			.number()
			.describe(
				"开仓金额（USDT保证金）- 该金额表示保证金数量，实际仓位 = 保证金 × 杠杆，例如25 USDT保证金配合10倍杠杆开250 USDT仓位",
			),
		isNotional: z
			.boolean()
			.optional()
			.default(false)
			.describe(
				"金额类型：false=USDT保证金（默认），true=USDT面值。注意：默认使用保证金模式",
			),
	}),
...
export async function executeClosePosition({
	...
	try {
		...
		const contractMultiplierForProvider = useContractUnits
			? await getQuantoMultiplier(contract)
			: 1;
		...
		const okxPosSideRaw =
			typeof okxPosition.posSide === "string"
				? okxPosition.posSide.toLowerCase()
				: "";
		const normalizedPosSide =
			okxPosSideRaw === "long" || okxPosSideRaw === "short"
				? okxPosSideRaw
				: "net";
		const marginModeRaw =
			typeof okxPosition.marginMode === "string"
				? okxPosition.marginMode.toLowerCase()
				: "";
		const marginModeForOrder =
			marginModeRaw === "isolated" ? "isolated" : "cross";

		// 🔒 按策略参数执行最小持仓时间检查，防止刚开仓就立即平仓
		if (!skipGuards && minHoldingMinutes > 0) {
			let openedTimestamp =
				toTimestampMillis(
					okxPosition.createTime ??
						okxPosition.cTime ??
						okxPosition.opened_at ??
						okxPosition.updateTime,
				);

			if (!openedTimestamp) {
				const dbPositionResult = await dbClient.execute({
					sql: `SELECT opened_at FROM positions
			       WHERE symbol = ? AND (account_id IS NULL OR account_id = ?)
				...
				if (dbPositionResult.rows.length > 0) {
					openedTimestamp = toTimestampMillis(
						dbPositionResult.rows[0].opened_at,
					);
				}
			}
			...
			if (openedTimestamp) {
				const holdingMinutes =
					(Date.now() - openedTimestamp) / (1000 * 60);
				if (holdingMinutes < minHoldingMinutes) {
					return fail(
						`拒绝平仓 ${normalizedSymbol}：持仓时间仅 ${holdingMinutes.toFixed(1)} 分钟，少于最小持仓时间 ${minHoldingMinutes.toFixed(1)} 分钟。请等待至少半个交易周期后再评估平仓。这是为了防止在同一周期内刚开仓就立即平仓，造成不必要的手续费损失。`,
					);
				}

				logger.info(
					`${normalizedSymbol} 持仓时间: ${holdingMinutes.toFixed(1)} 分钟，通过最小持仓时间检查`,
				);
			} else {
				logger.warn(
					`无法获取 ${normalizedSymbol} 的持仓时间戳，已跳过最小持仓时间检查（account=${accountId})`,
				);
			}
		}
		...
		const isFullClose =
			percentage >= 100 - 1e-6 || targetQuantity >= quantity - 1e-8;
		let closeSize = isFullClose
			? quantity
			: normalizeToStep(targetQuantity, "down");

		if (
			!isFullClose &&
			closeSize < minTradableSize &&
			quantity >= minTradableSize
		) {
		...
		const grossPnl = priceChange * closeSize * contractMultiplierForProvider;

		logger.info(
			`预估盈亏: ${grossPnl >= 0 ? "+" : ""}${grossPnl.toFixed(2)} USDT (价格变动: ${priceChange.toFixed(4)})`,
		);

		//  计算手续费（开仓 + 平仓）
		const openFee =
			entryPrice * closeSize * contractMultiplierForProvider * 0.0005;
		const closeFee =
			currentPrice * closeSize * contractMultiplierForProvider * 0.0005;
		...
		logger.info(
			`平仓 ${normalizedSymbol} ${side === "long" ? "做多" : "做空"} ${closeSize}${unitLabel} (入场: ${entryPrice.toFixed(2)}, 当前: ${currentPrice.toFixed(2)})`,
		);

		//  市价单平仓
		// 注意：币安双向持仓模式下，通过 positionSide + 反向 side 来平仓，不需要 reduceOnly 参数
		// 单向持仓模式（net）需要 reduceOnly
		// Gate 交易所若未标记 reduce_only，会把反向单当成新开仓并冻结额外保证金
		const forceReduceOnly = exchangeProvider === "gate";
		const isReduceOnly = forceReduceOnly || positionSideForOrder === "net";
		...
		if (order.id) {
			...
			while (retryCount < maxRetries) {
				try {
					...
					logger.info(
						`成交: ${actualCloseSize}${unitLabel} @ ${actualExitPrice.toFixed(2)} USDT`,
					);
					...
					if (priceDeviation > 0.03) {
						...
						logger.warn(
							`⚠️ 平仓成交价偏离超过3%: ${currentPrice.toFixed(2)} → ${actualExitPrice.toFixed(2)} (偏离 ${(priceDeviation * 100).toFixed(2)}%)`,
						);
					}
					...
					// 盈亏 = 价格变化 * 张数 * 合约乘数
					const grossPnl =
						priceChange * actualCloseSize * contractMultiplierForProvider;

					//  扣除手续费（开仓 + 平仓）
					// 开仓手续费 = 开仓名义价值 * 0.05%
					const openFee =
						entryPrice * actualCloseSize * contractMultiplierForProvider * 0.0005;
					// 平仓手续费 = 平仓名义价值 * 0.05%
					const closeFee =
						actualExitPrice * actualCloseSize * contractMultiplierForProvider * 0.0005;
					// 总手续费
					const totalFees = openFee + closeFee;
			...
			if (exchangeProvider === "binance" && order.id) {
				const exchangePnl = await getBinanceOrderRealizedPnl(
					client,
					contract,
					order.id.toString(),
				);
				if (exchangePnl !== null) {
					...
					logger.info(
						`采用 Binance realizedPnl: ${pnl >= 0 ? "+" : ""}${pnl.toFixed(2)} USDT`,
					);
				}
			}
		}
		...
		if (actualExitPrice === 0) {
			try {
				const ticker = await client.getFuturesTicker(contract);
				const tickerPrice = Number.parseFloat(
				...
				if (tickerPrice > 0) {
					...
					pnl =
						priceChangeFix * actualCloseSize * contractMultiplierForProvider -
						fixOpenFee -
						fixCloseFee;
		...
		// 🔥 关键验证：检查盈亏计算是否正确
		const notionalValue =
			actualExitPrice * actualCloseSize * contractMultiplierForProvider;
		const priceChangeCheck =
			side === "long"
				? actualExitPrice - entryPrice
				: entryPrice - actualExitPrice;
		const expectedPnl =
			priceChangeCheck * actualCloseSize * contractMultiplierForProvider -
			totalFee;

		// 检测盈亏是否被错误地设置为名义价值
		if (
			!usedExchangeRealizedPnl &&
			Math.abs(pnl - notionalValue) < Math.abs(pnl - expectedPnl)
		) {
			logger.error(`🚨 检测到盈亏计算异常！`);
			logger.error(
				`  当前pnl: ${pnl.toFixed(2)} USDT 接近名义价值 ${notionalValue.toFixed(2)} USDT`,
			);
			logger.error(`  预期pnl: ${expectedPnl.toFixed(2)} USDT`);
			logger.error(
				`  开仓价: ${entryPrice}, 平仓价: ${actualExitPrice}, 数量: ${actualCloseSize}, 合约乘数: ${contractMultiplierForProvider}`,
			);
			logger.error(
				`  价格变动: ${priceChangeCheck.toFixed(4)}, 手续费: ${totalFee.toFixed(4)}`,
			);
		...
		logger.info(
			`  开仓价: ${entryPrice.toFixed(4)}, 平仓价: ${actualExitPrice.toFixed(4)}, 数量: ${actualCloseSize}${unitLabel}`,
		);
		logger.info(
			`  价格变动: ${priceChangeCheck.toFixed(4)}, 合约乘数: ${contractMultiplierForProvider}`,
		);
		logger.info(
			`  毛盈亏: ${(priceChangeCheck * actualCloseSize * contractMultiplierForProvider).toFixed(2)} USDT`,
		);
		logger.info(
			`  开仓手续费: ${dbOpenFee.toFixed(4)} USDT, 平仓手续费: ${dbCloseFee.toFixed(4)} USDT`,
		);
...
export const cancelOrderTool = createTool({
	...
	execute: async ({ orderId }) => {
		...
		try {
			...
			const cancelResult: any = await client.cancelOrder(
