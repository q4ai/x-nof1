...
export class BitgetClient {
	...
	private async request<T>(
		...
		try {
			...
			if (!response.ok) {
				throw new Error(
					`Bitget API request failed: ${response.status} ${response.statusText} - ${text}`,
				);
			}
	...
	async getFuturesTicker(contract: string) {
		...
		return {
			...
			change_percentage: Number.parseFloat(ticker.change24h) * 100, // Bitget returns decimal
	...
	async getFuturesCandles(contract: string, interval = "5m", limit = 100) {
		...
		const granularity =
			granularityMap[interval.toLowerCase()] ||
			granularityMap[interval] ||
			"5m";
	...
	async getPositions() {
		...
		return data.map((pos: any) => {
			...
			const size = Number.parseFloat(pos.total); // Total position size
		...
		for (const ticker of tickers || []) {
			if (ticker.symbol && ticker.symbol.endsWith("USDT")) {
				...
				const volume24h = Number.parseFloat(
					ticker.usdtVolume || ticker.quoteVolume || "0",
				);
				const price = ticker.lastPr || ticker.last || "0";
				// Bitget 的 change24h 可能是小数形式（如 0.05 表示 5%），需要乘以 100
				const rawChange = Number.parseFloat(
					ticker.change24h || ticker.priceChangePercent || "0",
				);
	...
	async setLeverage(
		...
		try {
			...
			logger.warn(
				`Set leverage/margin mode failed for ${contract}: ${error.message}`,
			);
			return null;
		}
	}
