...
export class OkxClient {
	...
	> {
		...
		if (auth) {
			...
			headers["OK-ACCESS-SIGN"] = await this.sign(
				timestamp,
				method,
				requestPath,
				payload,
		...
		try {
			...
			if (!response.ok) {
				...
				const error: any = new Error(
					`OKX API 请求失败: ${response.status} ${response.statusText} - ${text}`,
				);
			...
			if (json.code && json.code !== "0") {
				const error: any = new Error(
					`OKX API 错误: ${json.code} - ${json.msg || json.data?.[0]?.sMsg || "未知错误"}`,
				);
	...
	async getFuturesTicker(contract: string) {
		...
		const changePercentage =
			safeNumber(last) && safeNumber(open24h)
				? ((safeNumber(last) - safeNumber(open24h)) /
						Math.max(safeNumber(open24h), 1e-8)) *
					100
				: 0;
	...
	async getFundingRate(contract: string) {
		const instId = this.contractToInstrument(contract);
		const [rate] = await this.request<any[]>(
			"GET",
	...
	async getFuturesAccount() {
		const data = await this.request<any[]>("GET", "/api/v5/account/balance", {
			ccy: "USDT",
		});
		const first = data?.[0];
	...
	async getPositions(): Promise<OkxPosition[]> {
		const positions = await this.request<RawPosition[]>(
			"GET",
		...
		return positions.map((pos) => {
			...
			const marginMode =
				(pos.mgnMode ?? "cross").toLowerCase() === "isolated"
					? "isolated"
					: "cross";
	...
	async placeOrder(params: OrderParams) {
		...
		const result = {
			...
			left: order.accFillSz
				? String(sizeAbs - safeNumber(order.accFillSz, 0))
				: String(sizeAbs),
			instId,
	...
	async getOrder(orderId: string, contract?: string, _clientOrderId?: string) {
		const resolvedContract =
			contract || (await this.resolveContractForOrder(orderId));
		if (!resolvedContract) {
	...
	async getOrderHistory(contract?: string, limit = 10) {
		...
		(orders || []).forEach((order) => {
			if (order?.ordId) {
				this.orderContractCache.set(
					order.ordId,
					this.instrumentToContract(order.instId),
				);
			}
		});
		return orders ?? [];
	...
	async getMyTrades(contract?: string, limit = 10) {
		...
		(trades || []).forEach((trade) => {
			if (trade?.ordId) {
				this.orderContractCache.set(
					trade.ordId,
					this.instrumentToContract(trade.instId),
				);
			}
		});
		return trades ?? [];
	...
	private async resolveContractForOrder(
		orderId: string,
...
type ExchangeHttpClient = OkxClient | BinanceClient | BitgetClient | GateClient;
...
async function loadActiveAccountConfig(): Promise<{
	...
	try {
		...
		if (activeAccount) {
			logger.info(
				`使用数据库活跃账户: ${activeAccount.name} (${activeAccount.provider})`,
			);
			return {
				provider: activeAccount.provider as
					| "okx"
					| "binance"
					| "bitget"
					| "gate",
...
function buildExchangeClient(): ExchangeHttpClient {
	...
	if (credentials.provider === "binance") {
		if (!credentials.apiKey || !credentials.apiSecret) {
			throw new Error(
				"请在环境变量或数据库配置中设置 BINANCE_API_KEY / BINANCE_API_SECRET",
			);
		}
	...
	if (credentials.provider === "bitget") {
		if (!credentials.apiKey || !credentials.apiSecret) {
			throw new Error(
				"请在环境变量或数据库配置中设置 BITGET_API_KEY / BITGET_API_SECRET / BITGET_API_PASSPHRASE",
			);
		}
		const passphrase =
			"passphrase" in credentials ? credentials.passphrase : "";
		const simulated =
			"simulated" in credentials ? credentials.simulated : false;
	...
	if (credentials.provider === "gate") {
		if (!credentials.apiKey || !credentials.apiSecret) {
			throw new Error(
				"请在环境变量或数据库配置中设置 GATE_API_KEY / GATE_API_SECRET",
			);
		}
		const testnet = "testnet" in credentials ? credentials.testnet : false;
	...
	if (!credentials.apiKey || !credentials.apiSecret) {
		throw new Error(
			"请在环境变量或数据库配置中设置 OKX_API_KEY / OKX_API_SECRET / OKX_API_PASSPHRASE",
		);
	}

	const passphrase = "passphrase" in credentials ? credentials.passphrase : "";
	const simulated = "simulated" in credentials ? credentials.simulated : false;
...
export async function initExchangeClient(): Promise<ExchangeHttpClient> {
	...
	if (config.provider === "binance") {
		...
		clientInstance = new BitgetClient(
			config.apiKey,
			config.apiSecret,
			config.passphrase,
			config.simulated,
	...
	} else if (config.provider === "gate") {
		if (!config.apiKey || !config.apiSecret) {
			throw new Error("活跃账户缺少 Gate.io API Key 或 API Secret");
		}
		clientInstance = new GateClient(
			config.apiKey,
			config.apiSecret,
			config.testnet,
			proxyUrl,
...
export function createExchangeClientForAccount(account: {
	...
	if (account.provider === "gate") {
		if (!account.api_key || !account.api_secret) {
			throw new Error("账户缺少 Gate.io API Key 或 API Secret");
		}
		return new GateClient(
			account.api_key,
			account.api_secret,
