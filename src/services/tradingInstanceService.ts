 ...
 * 获取所有 Strategy Tasks
 */
export async function getAllTradingInstances(): Promise<TradingInstanceWithDetails[]> {
  try {
    const result = await dbClient.execute(`
      SELECT 
        ti.*,
 ...
 */
export async function getRunningInstances(): Promise<TradingInstanceWithDetails[]> {
  try {
    const result = await dbClient.execute(`
      SELECT 
        ti.*,
 ...
 */
export async function getTradingInstanceById(id: number): Promise<TradingInstanceWithDetails | null> {
  try {
    const result = await dbClient.execute({
      sql: `
        SELECT 
 ...
 * 创建 Strategy Task
 */
export async function createTradingInstance(input: CreateTradingInstanceInput): Promise<TradingInstance> {
  try {
...
export async function getLatestInstance(): Promise<TradingInstanceWithDetails | null> {
  try {
    ...
    return {
      ...
      last_execution_status: row.last_execution_status ? String(row.last_execution_status) as any : null,
      created_at: String(row.created_at),
      updated_at: String(row.updated_at),
      account_name: row.account_name ? String(row.account_name) : undefined,
      account_provider: row.account_provider ? String(row.account_provider) : undefined,
 ...
 */
export async function getLatestInstanceForAccount(accountId: number): Promise<TradingInstanceWithDetails | null> {
  try {
    const result = await dbClient.execute({
      sql: `
        SELECT 
