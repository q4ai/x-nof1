...
export async function getAllAccounts(): Promise<AccountConfig[]> {
	try {
		const result = await dbClient.execute(
			"SELECT * FROM account_configs ORDER BY created_at DESC",
		);
		return result.rows.map((row: any) => ({
			...
			stop_loss_usdt: row.stop_loss_usdt
				? Number(row.stop_loss_usdt)
				: undefined,
			take_profit_usdt: row.take_profit_usdt
				? Number(row.take_profit_usdt)
				: undefined,
...
export async function migrateFromEnv(): Promise<void> {
	try {
		...
		const stopLossUsdt = process.env.ACCOUNT_STOP_LOSS_USDT
			? Number(process.env.ACCOUNT_STOP_LOSS_USDT)
			: undefined;
		const takeProfitUsdt = process.env.ACCOUNT_TAKE_PROFIT_USDT
			? Number(process.env.ACCOUNT_TAKE_PROFIT_USDT)
			: undefined;
		...
		const account = await createAccount({
			...
			take_profit_usdt: Number.isFinite(takeProfitUsdt)
				? takeProfitUsdt
				: undefined,
		});
