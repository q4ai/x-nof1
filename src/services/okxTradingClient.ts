import { createOkxClient, type OkxPosition } from "./okxClient";
...
class OkxTradingClient {
  ...
  async getFuturesAccount(): Promise<FuturesAccountSnapshot> {
    ...
    const positionMargin = snapshot.positionMargin ?? String(Math.max(total - available, 0));
    return {
  ...
  async getFuturesCandles(contract: string, interval = "5m", limit = 100): Promise<FuturesCandle[]> {
    const candles = await this.client.getFuturesCandles(contract, interval, limit);
  ...
  const side = order.side === "buy" || order.posSide === "long" ? "long" : "short";
      ...
      return {
        ...
        is_reduce_only: order.reduceOnly === "true" || order.reduceOnly === true,
