...
class WebSocketService {
	...
	broadcast(
		...
		if (sentCount > 0) {
			logger.debug(
				`WebSocket 消息已广播到 ${sentCount} 个客户端: ${message.type}`,
			);
		}
	}
	...
	private buildStatusCacheKey(message: TradingStatusMessage): string {
		if (message?.data && typeof message.data === "object") {
			const instanceId = this.extractNumericId(
				message.data.instanceId ?? message.data.instance_id,
			);
			if (instanceId !== null) {
				return `instance:${instanceId}`;
			}

			const accountId = this.extractNumericId(
				message.data.accountId ?? message.data.account_id,
			);
			if (accountId !== null) {
