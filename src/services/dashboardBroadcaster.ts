...
const PRICE_INTERVAL_MS = Number.parseInt(
	process.env.DASHBOARD_PRICE_INTERVAL_MS || "10000",
	10,
);
const POSITION_INTERVAL_MS = Number.parseInt(
	process.env.DASHBOARD_POSITION_INTERVAL_MS || "15000",
	10,
);
const CANDLE_INTERVAL_MS = Number.parseInt(
	process.env.DASHBOARD_CANDLE_INTERVAL_MS || "20000",
	10,
);
const CANDLE_LIMIT = Number.parseInt(
	process.env.DASHBOARD_CANDLE_LIMIT || "200",
	10,
);
...
function normalizeLimit(limitRaw: unknown): number {
	if (typeof limitRaw === "number" && Number.isFinite(limitRaw)) {
		return Math.min(
			Math.max(Math.floor(limitRaw), MIN_CANDLE_LIMIT),
			MAX_CANDLE_LIMIT,
		);
	}
...
class DashboardBroadcaster {
	...
	private subscribeCandles(
		...
		if (existingTask) {
			...
			this.sendCachedOrFetchCandles(
				existingTask.fetch,
				key,
				symbol,
				intervalNormalized.api,
				limit,
				client,
			);
		} else {
			const task = this.createCandleTask(
				key,
				symbol,
				intervalNormalized.api,
				limit,
			...
			try {
				...
				const recipients =
					targets && targets.size > 0
						? targets
						: this.candleSubscribers.get(key);
	...
	private async fetchAndBroadcastPrices(
		...
		try {
			...
			const recipients =
				targets && targets.size > 0 ? targets : this.priceSubscribers;
	...
	private async fetchAndBroadcastPositions(
		targets?: Set<WebSocketClient>,
	): Promise<void> {
		...
		try {
			...
			const recipients =
				targets && targets.size > 0 ? targets : this.positionSubscribers;
			if (recipients.size > 0) {
				this.sendToClients(recipients, message);
			}
		} catch (error) {
	...
	private sendToClients(
		clients: Iterable<WebSocketClient>,
