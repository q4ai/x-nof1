...
import { existsSync, mkdirSync, readdirSync, statSync, unlinkSync, createWriteStream, createReadStream, copyFileSync } from "node:fs";
...
export async function listBackups(): Promise<BackupInfo[]> {
  ...
  try {
    ...
    const backups: BackupInfo[] = files
      .filter(file => file.endsWith(".zip") || file.endsWith(".db") || file.endsWith(".sqlite") || file.endsWith(".backup"))
      .map(name => {
...
export async function createBackup(): Promise<BackupInfo> {
  ...
  try {
    ...
    await new Promise<void>((resolve, reject) => {
      ...
      output.on("close", () => {
        logger.info(`备份压缩完成，总大小: ${formatFileSize(archive.pointer())}`);
        resolve();
      });
      ...
      if (existsSync(strategiesDir)) {
        const strategyFiles = readdirSync(strategiesDir).filter(f => f.endsWith(".json"));
    ...
    if (existsSync(dbPath)) {
      ...
      await new Promise<void>((resolve, reject) => {
        ...
        if (existsSync(strategiesDir)) {
          const strategyFiles = readdirSync(strategiesDir).filter(f => f.endsWith(".json"));
          for (const file of strategyFiles) {
            archive.file(join(strategiesDir, file), { name: `data/strategies/${file}` });
          }
        }
