...
export async function installSystem(data: InstallData): Promise<InstallResult> {
	...
	try {
		...
		const stopLossValue =
			typeof data.accountConfig.stop_loss_usdt === "number" &&
			Number.isFinite(data.accountConfig.stop_loss_usdt)
				? data.accountConfig.stop_loss_usdt
				: null;
		const takeProfitValue =
			typeof data.accountConfig.take_profit_usdt === "number" &&
			Number.isFinite(data.accountConfig.take_profit_usdt)
				? data.accountConfig.take_profit_usdt
				: null;
		const httpProxy = data.systemConfig.http_proxy_url?.trim() ?? "";
		const communityReportEnabled = Boolean(
			data.systemConfig.community_report_enabled,
		);
		const communitySharePrompts = communityReportEnabled
			? Boolean(data.systemConfig.community_share_prompts)
			: false;
		...
		const sysConfigs = [
			{
				key: "INITIAL_BALANCE",
				value: String(data.systemConfig.initial_balance),
			},
			{
				key: "TRADING_INTERVAL_MINUTES",
				value: String(data.systemConfig.trading_interval),
			},
			...
			{
				key: "COMMUNITY_REPORT_ENABLED",
				value: communityReportEnabled ? "true" : "false",
			},
			{
				key: "COMMUNITY_SHARE_PROMPTS",
				value: communitySharePrompts ? "true" : "false",
			},
			// 默认值
      ...
      `,
		...
		const passwordHash = crypto
			.createHash("sha256")
			.update(adminCredentials.password)
			.digest("hex");
