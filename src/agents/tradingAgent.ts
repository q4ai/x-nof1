...
import { getPromptTemplate } from "../prompts/templateLoader";
...
let instructionsTemplateCache: string | null = null;
let promptTemplateCache: string | null = null;

async function loadInstructionsTemplate(): Promise<string> {
	if (!instructionsTemplateCache) {
		instructionsTemplateCache = await getPromptTemplate("instructions_zh.txt");
	}
	return instructionsTemplateCache;
}

async function loadPromptTemplate(): Promise<string> {
	if (!promptTemplateCache) {
		promptTemplateCache = await getPromptTemplate("prompts_zh.txt");
	}
...
function buildBasePromptVariables(
	...
	const symbolList = RISK_PARAMS.TRADING_SYMBOLS.join("、");
	return {
...
export async function getStrategyPromptDefaultSections(
	...
	const baseVariables = buildBasePromptVariables(strategy, intervalMinutes, riskConfig, profile.label);
...
function formatMarketDataSection(marketData: Record<string, any>): string {
...
function formatAccountSection(accountInfo: any, positions: any[], intervalMinutes: number): string {
	if (!accountInfo) {
		return "账户信息缺失";
	}
...
function formatTradeHistorySection(tradeHistory?: any[]): string {
	...
	for (const trade of tradeHistory.slice(-10)) {
...
function formatRecentDecisionsSection(recentDecisions?: any[]): string {
...
export async function generateTradingPrompt(input: TradingPromptInput): Promise<string> {
	...
	sectionsOutput.push(formatMarketDataSection(marketData));
	sectionsOutput.push(formatAccountSection(accountInfo, positions, intervalMinutes));
	const tradeSection = formatTradeHistorySection(tradeHistory);
	if (tradeSection) {
		sectionsOutput.push(tradeSection);
	}
	const decisionsSection = formatRecentDecisionsSection(recentDecisions);
	if (decisionsSection) {
...
async function generateInstructions(strategy: TradingStrategy, intervalMinutes: number): Promise<string> {
	...
	const template = await loadInstructionsTemplate();
