...
export const RISK_PARAMS = {
	...
	get ACCOUNT_DRAWDOWN_NO_NEW_POSITION_PERCENT(): number {
		return Number.parseInt(
			getConfig("ACCOUNT_DRAWDOWN_NO_NEW_POSITION_PERCENT", "30"),
			10,
		);
	},
	...
	get ACCOUNT_DRAWDOWN_FORCE_CLOSE_PERCENT(): number {
		return Number.parseInt(
			getConfig("ACCOUNT_DRAWDOWN_FORCE_CLOSE_PERCENT", "50"),
			10,
		);
	},
	...
	get ACCOUNT_DRAWDOWN_WARNING_PERCENT(): number {
		return Number.parseInt(
			getConfig("ACCOUNT_DRAWDOWN_WARNING_PERCENT", "20"),
			10,
		);
	},
	...
	get PROMPT_LANGUAGE(): StrategyLanguage {
		return normalizeStrategyLanguage(
			getConfig("PROMPT_LANGUAGE", DEFAULT_STRATEGY_LANGUAGE),
		);
	},
 ...
 */
export function getConfigStringValue(
	key: string,
	defaultValue: string,
): string {
	const value = getConfig(key, defaultValue);
